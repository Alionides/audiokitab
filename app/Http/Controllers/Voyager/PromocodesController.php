...
class PromocodesController extends VoyagerBaseController
{
    //***************************************
    //
    //                   /\
    ...
    {
        $number = Str::random(6);
        if (Promocodelist::where('code', $number)->count() > 0) self::generateNumber();
        return strtoupper($number);
    }

    public function store(Request $request)
    {
        //return response($request->subscribe_id);
        //return response($request->count);
        // return response($this->generateNumber());
        $count = $request->count;
        $subscribe_id = $request->subscribe_id;

        for ($i=0; $i <= $count; $i++) { 
            
            $data = new Promocodelist;
